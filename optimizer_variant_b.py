"""
Optimizer Variant B - Force VIX Regime Enabled
===============================================
Tests if VIX regime adaptation helps when forced on.
"""
import os
import sys
...
def create_objective_vix(min_trades: int = 30):
    def objective(trial: optuna.Trial) -> float:
        ...
        params = {
            ...
            # VIX regime - FORCED ON
            "use_vix_regime": True,  # FORCED ON
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Optimizer B - Force VIX Regime")
    ...
    parser.add_argument("--save", type=str, default="best_vix.json")
    ...
    print("=" * 60)
    print("  OPTIMIZER B - FORCE VIX REGIME ENABLED")
    print("  (use_vix_regime=True, optimize thresholds & multipliers)")
    print("=" * 60)
    ...
    study.optimize(
        create_objective_vix(args.min_trades),
    ...
    if BEST_RESULT:
        print("\n" + "=" * 60)
        print("  BEST RESULT (VIX REGIME FORCED)")
        print("=" * 60)
        ...
        print(f"\n  VIX Settings:")
        print(f"    vix_high_threshold: {p.get('vix_high_threshold')}")
        print(f"    vix_low_threshold: {p.get('vix_low_threshold')}")
        print(f"    high_vol_cooldown_mult: {p.get('high_vol_cooldown_mult'):.2f}")
        print(f"    low_vol_cooldown_mult: {p.get('low_vol_cooldown_mult'):.2f}")
